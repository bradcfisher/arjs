...
43CF: ED 47 63 PROCESS_HOURLY_UPDATES
                        SBC CHR_HP_MAX_HI   ; Subtract max HP HI byte from A with carry (presumably A=0, no carry)
...
441A: B0 02             BCS L441E           ; If no borrow (A>=0), skip next instruction
441C: A9 00             LDA #$00            ; Store 0 in A (clamp effective value to 0)
441E: A0 00   L441E     LDY #$00            ; Store 0 in Y
...
4430: AD 79 63          LDA $6379           ; Load value at $6379 (unknown char stat effective value - possibly character SPD)
4433: 4A                LSR                 ; Divide A by 16
4434: 4A                LSR                 ;   This produces a value ranging
4435: 4A                LSR                 ;   from 0 to 15 (255/16 = 15)
4436: 4A                LSR                 ;   That value is then used as an index into a table @ $445F
...
443E: 2C 98 63          BIT CHR_STOMACH     ; AND accumuator with ?CHR_STOMACH???  TODO: Why?
4441: 10 03             BPL $4446           ; Skip next instruction if sign bit is not set
...
445E: 60                RTS
; The following table is used to lookup a value based on the CHR_UNK_EFFECTIVE value
445F: UNK_STAT_TBL
        .BYTE $06
        .BYTE $07
        .BYTE $08
